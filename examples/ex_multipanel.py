...
data = {
            ...
            "timeseries2": np.array(
                [
                    np.cos(np.arange(0, 25, 0.1)) + np.random.randn(250)
                    for _ in range(20)
                ]
            ),
...
def timeseries(axis, y, true=False, c='black'):
    ...
    axis.fill_between(x, y.mean(0)-std, y.mean(0)+std, zorder=0, facecolor=c,
                      alpha=0.3)
...
fig = MultiPanel(grid=(4, 2), figsize=(9, 4), labels=True, label_location=(-0.2, 1.05),
                 label_size=10, wspace=0.35, hspace=0.35)
